...
const errorsToXml: TErrorsToXml = (errors) => {
    const inner: string = errors
        .map(
            (err) =>
                "  <error"
                + ' rule="' + escapeXml(err.ruleId)
                + '"'
                + ' line="' + String(err.line)
                + '"'
                + ' col="' + String(err.column)
                + '"'
                + ' message="'
...
type TPlanOptionToXml = (
    option: TPlanOption,
) => string;

const planOptionToXml: TPlanOptionToXml = (
    option,
) => {
    const id: string = String(option.id);
    const desc: string =
        "  <description>"
        + escapeXml(option.description)
        + "</description>";
    const solves: string =
    ...
    const differs: string =
        ...
        + (option.differs_from_previous === null
            ? "null"
            : escapeXml(
                  option.differs_from_previous,
              ))
        + "</differs_from_previous>";
    return '<chosen_option id="' + id + '">\n'
        + desc + "\n"
        + solves + "\n"
        + regression + "\n"
        + emergent + "\n"
...
type TTargetRuleToXml = (
    triage: TTriageResult,
) => string;

const targetRuleToXml: TTargetRuleToXml = (
    triage,
) => {
