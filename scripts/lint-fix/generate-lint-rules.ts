...
import { LINT_META as preferCallSignature } from "../../type-based/prefer-call-signature";
...
const customRules: ReadonlyArray<TRuleEntry> = [
    ["local/max-total-depth", maxTotalDepth],
    ["local/no-nested-function", noNestedFunction],
    ["local/restrict-return-count", restrictReturnCount],
    ["local/enforce-record-type", enforceRecordType],
    ["local/max-type-nesting", maxTypeNesting],
    ["local/no-duplicate-type-structure", noDuplicateTypeStructure],
    ["local/no-single-field-type", noSingleFieldType],
    ["local/prefer-call-signature", preferCallSignature],
    ["local/require-extracted-function-type", requireExtractedTypes],
    ["local/require-parametric-record", requireParametricRecord],
    ["local/valid-generics", validGenerics],
    ["local/require-rest-params-tuple", requireRestParamsTuple],
    ["local/finite-domain-return-widening", finiteDomainReturnWidening],
    ["local/type-distance", typeDistance],
    ["local/cardinality-isomorphic-families", cardinalityIsomorphicFamilies],
    ["local/transport-graph", transportGraph],
    ["local/fiber-coherence", fiberCoherence],
];

type TRuleXmlBuilder = (entry: TRuleEntry) => string;

const buildRuleXml: TRuleXmlBuilder = (entry) => {
    const [name, meta]: TRuleEntry = entry;
    const flags: string = "    <flags>" + escapeXml(meta.flags) + "</flags>";
    const fix: string = "    <fix>" + escapeXml(meta.fix) + "</fix>";
    return (
        '  <rule name="' +
        escapeXml(name) +
        '">\n' +
...
const buildLintRulesSection: TXmlBuilder = () => {
    const externalEntries: ReadonlyArray<TRuleEntry> = Array.from(
        externalRegistry.entries(),
    );
